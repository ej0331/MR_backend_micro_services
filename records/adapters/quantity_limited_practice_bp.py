...
from usecases.quantity_limited_practice_service import QuantityLimitedPracticeService
from validations.record_index_schema import RecordIndexSchema
from validations.quantity_limited_practice_store_schema import QuantityLimitedPracticeStoreSchema
from validations.quantity_limited_practice_update_schema import QuantityLimitedPracticeUpdateSchema
...
@quantity_limited_practice_blueprint.route('/quantity_limited_practices/health-check', methods=['GET'])
@login_required
def health_check():
...
def store():
    ...
    try:
        validated_data = QuantityLimitedPracticeStoreSchema().load(data)
    except ValidationError as err:
        for message in err.messages:
            messages.append({
                message: err.messages[message][0]
