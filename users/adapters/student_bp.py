...
from validations.student_index_schema import UserIndexSchema
from validations.student_store_schema import UserStoreSchema
from validations.student_show_schema import UserShowSchema
from validations.student_update_schema import UserUpdateSchema
...
def index():
    ...
    try:
        response, total = student_service.get_users(
...
def store():
    ...
    try:
        response = student_service.insert_user(
            name=validated_data.get('name'),
            account=validated_data.get('account'),
...
def show(id):
    ...
    try:
        response = student_service.get_user(
...
def update(id):
    ...
    try:
        response = student_service.update_user(
            id=validated_data.get('id'),
            class_id=validated_data.get('class_id'),
            account=validated_data.get('account'),
            name=validated_data.get('name'),
...
def delete(id):
    ...
    try:
        response = student_service.delete_user(
            id=validated_data.get('id'),
        )
        return make_success_response(response, 200)
    except Exception as e:
