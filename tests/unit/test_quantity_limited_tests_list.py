...
scenarios('../features/unit/quantity_limited_test_list.feature')
...
@when(
    parsers.parse(
        '發送 get 請求至後端 endpoint /api/quantity_limited_tests?page={page}&per_page={per_page}'),
...
def when_send_uantitly_limited_prectice_request(client, given_get_quantitly_limited_prectice, page, per_page):
    ...
    response = client.get(f'/api/quantity_limited_tests?page={page}&per_page={per_page}', json=given_get_quantitly_limited_prectice,
