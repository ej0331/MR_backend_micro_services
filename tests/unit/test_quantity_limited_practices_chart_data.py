...
scenarios('../features/unit/quantity_limited_practices_chart_data.feature')
...
@when(
    parsers.parse(
        '發送 get 請求至後端 endpoint /api/quantity_limited_practices/chart/users/{id}'),
...
def when_send_get_data_request(client, given_valid_parameters, id):
    ...
    response = client.get(f'/api/quantity_limited_practices/chart/users/{id}', json=given_valid_parameters,
...
def then_check_response(when_send_get_data_request):
    ...
    assert len(data["level1_times"]) == 5
    assert len(data["level1_total_quantities"]) == 5
    assert len(data["level2_finished_at_list"]) == 5
    assert len(data["level2_times"]) == 5
    assert len(data["level2_total_quantities"]) == 5
    assert len(data["level3_finished_at_list"]) == 5
    assert len(data["level3_times"]) == 5
    assert len(data["level3_total_quantities"]) == 5
