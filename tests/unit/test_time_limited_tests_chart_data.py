...
scenarios('../features/unit/time_limited_tests_chart_data.feature')
...
@when(
    parsers.parse(
        '發送 get 請求至後端 endpoint /api/time_limited_tests/chart/users/{id}'),
...
def when_send_get_data_request(client, given_valid_parameters, id):
    ...
    response = client.get(f'/api/time_limited_tests/chart/users/{id}', json=given_valid_parameters,
...
def then_check_response(when_send_get_data_request):
    ...
    assert len(data["level1_time_limit_list"]) == 5
    assert len(data["level1_time_list"]) == 5
    assert len(data["level1_total_quantity_list"]) == 5
    ...
    assert len(data["level2_time_limit_list"]) == 5
    assert len(data["level2_time_list"]) == 5
    assert len(data["level2_total_quantity_list"]) == 5
    ...
    assert len(data["level3_time_limit_list"]) == 5
    assert len(data["level3_time_list"]) == 5
    assert len(data["level3_total_quantity_list"]) == 5
