...
scenarios('../features/unit/time_limited_tests.feature')
...
def given_valid_parameters(name, type_id_list, finished_start, finished_end, class_id_list, page, per_page):
...
@when(
    parsers.parse(
        '發送 get 請求至後端 endpoint /api/time_limited_test?page={page}&per_page={per_page}'),
...
def when_send_get_data_request(client, given_valid_parameters, page, per_page):
    headers = {
        'Cookie': f'session={get_session_value(client)}'
    }
    response = client.get(f'/api/time_limited_tests?page={page}&per_page={per_page}', json=given_valid_parameters,
...
def then_check_response(when_send_get_data_request):
    ...
    assert json_response.get('current_page') is not None
    assert json_response.get('max_page') is not None
    assert json_response.get('messages') is None
    assert json_response.get('per_page') is not None
    assert json_response.get('to') is not None
    ...
    if isinstance(data, list):
        for sub_item in data:
            ...
            assert sub_item.get('level1_correct_quantity') is not None
            assert sub_item.get('level1_incorrect_quantity') is not None
            assert sub_item.get('level1_time') is not None
            assert sub_item.get('level1_time_limit') is not None
            assert sub_item.get('level1_total_quantity') is not None
            assert sub_item.get('level2_correct_quantity') is not None
            assert sub_item.get('level2_incorrect_quantity') is not None
            assert sub_item.get('level2_time') is not None
            assert sub_item.get('level2_time_limit') is not None
            assert sub_item.get('level2_total_quantity') is not None
            assert sub_item.get('level3_correct_quantity') is not None
            assert sub_item.get('level3_incorrect_quantity') is not None
            assert sub_item.get('level3_time') is not None
            assert sub_item.get('level3_time_limit') is not None
