...
scenarios('../features/unit/time_limited_test_list.feature')


@given(
    parsers.parse(
        '題目類型:{type_id_list},查詢的起始時間:{finished_start}, 查詢的結束時間:{finished_end},班級:{class_id_list},頁碼:{page},每頁呈現比數:{per_page}'),
...
def given_get_quantitly_limited_prectice(type_id_list, finished_start, finished_end, class_id_list, page, per_page):
...
@when(
    parsers.parse(
        '發送 get 請求至後端 endpoint /api/time_limited_tests?page={page}&per_page={per_page}'),
...
def when_send_uantitly_limited_prectice_request(client, given_get_quantitly_limited_prectice, page, per_page):
    ...
    response = client.get(f'/api/time_limited_tests?page={page}&per_page={per_page}', json=given_get_quantitly_limited_prectice,
...
def then_check_response(when_send_uantitly_limited_prectice_request):
    ...
    assert json_response.get('per_page') == len(data)
