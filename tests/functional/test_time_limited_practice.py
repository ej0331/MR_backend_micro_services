...
scenario = partial(scenario, "../features/functional/time_limited_practice.feature")
selected_class = ""
selected_question_type = ""
selected_start_date = ""
selected_end_date = ""

@scenario("透過班級篩選限時練習列表")
...
def when_click_quantity_limited(driver):
    time_limited_tab = WebDriverWait(driver, 5).until(EC.presence_of_element_located(
        (By.XPATH, '/html/body/div/div/div[2]/div[2]/div[1]/div/div[2]')))
    time_limited_tab.click()
...
@then('確認限時練習列表僅有此姓名學生的限時練習作答記錄', target_fixture='then_check_table_result')
...
def then_check_table_result(driver):
    ...
    rows = tbody.find_elements(By.TAG_NAME, 'tr')
    for row in rows:
        row_datetime = datetime.strptime(row.find_element(By.XPATH, 'td[17]').text, "%Y/%m/%d %H:%M")
